...
def train(args, local_rank, dataset, ctx_tokenizer, save_path='./'):
    ...
    if local_rank == 0:
        print(f"Creating the dataloaders...")
...
def main():
    ...
    print("Encoding the datasets")
    processes = []
    ctx = mp.get_context('spawn')
    for rank in range(args.n_gpu):
        print(f"=> Loading the dataset shard {rank}.")
        ...
        p = ctx.Process(target=train, args=(args, rank, sub_dataset, ctx_tokenizer, args.output_dir + f"/encoded/wikipedia_split_with_title_shard_{rank}"))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()

    print("Aggregating the shards ...")
    prefix = args.output_dir + "/encoded/wikipedia_split_with_title_shard_"
    dataset_shards = []
    for rank in range(args.n_gpu):
    ...
    print("Saving the encoded dataset to disk ...")
    os.makedirs(args.output_dir + "/databases/", exist_ok=True)
    encoded_dataset.add_faiss_index("embeddings")
    encoded_dataset.save_faiss_index("embeddings", args.output_dir + "/databases/" +  args.output_file_name + ".faiss")
    print("Done")
