...
filelog = logging.FileHandler('./logs/test_reader_'+datetime.now().strftime("%m_%d_%H_%M_%S")+'.log')
...
class ReaderTrainer(object):
    def __init__(self, args):
        ...
        self.best_validation_result = 0.0
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler):
        ...
        if save_cp:
            ...
            if reader_validation_score > self.best_validation_result:
    ...
    def validate(self):
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            ...
            pred_time += (datetime.now() - ist).microseconds
            if (i + 1) % log_result_step == 0:
                logger.info('Eval step: %d ', i)
        logger.info('Pure time used %d seconds.' % pred_time)
        logger.info('Test time used: %d seconds.' % (datetime.now() - st).seconds)
        ...
        for q_predictions in all_results:
            ...
            for (n, span_prediction) in span_predictions.items():
                scores = [exact_match_score(span_prediction.prediction_text, ga) for ga in gold_answers]
                if len(scores) > 0:
                    em_hit = max(scores)
                else:
                    em_hit = 0.0
    ...
    def _remove_emb_position_ids(self, state_dict):
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        ...
        if saved_state.model_dict:
            ...
            model_to_load.load_state_dict(self._remove_emb_position_ids(saved_state.model_dict))
...
def main():
    ...
    tester = ReaderTrainer(args)

    if args.dev_file:
        logger.info("No train files are specified. Run validation.")
        tester.validate()
    else:
        logger.warning("No (model_file & dev_file) parameters are specified. Nothing to do.")


def check_answer_differences():
    with open('/u/pandu/data/openQA/data/datasets/trec100_full/nq-test-rank.json', 'r') as t100f:
        trec100 = json.load(t100f)

    with open('/u/pandu/data/openQA/data/datasets/trec/nq-test-rank.json', 'r') as ttf:
        trec50 = json.load(ttf)

    qa100 = {}
    for que in trec100:
        qa100[que['question']] = que['answers']
    ...
    for que in trec50:
        ...
        answers = que['answers']
        if question in qa100:
            foundq += 1
            if len(answers) != len(qa100[question]):
                diff_answer += 1
                print('50:',answers, '\n100:', qa100[question])
    print('foundq', foundq, 'out of', len(trec50), len(trec100), 'diff_answer', diff_answer)


def complete_answer_based_on_differences():
    with open('/u/pandu/data/openQA/data/datasets/trec100_full/nq-test-rank.json', 'r') as t100f:
        trec100 = json.load(t100f)
    ...
    for que in trec50:
        ...
        answers = que['answers']
        if question in qa100:
            new_que["question"] = question
            new_que["ctxs"] = que["ctxs"]
            if len(answers) < len(qa100[question]):
                new_que['answers'] = qa100[question]
            else:
                new_que['answers'] = answers

        new_trec50.append(new_que)
    ofn = '/u/pandu/data/openQA/data/datasets/trec/nq-test-rank-full.json'
    print("Writing data in nq format to file", ofn)
    with open(ofn, 'w+') as ofw:
        ofw.write(json.dumps(new_trec50, indent=4) + '\n')


if __name__ == "__main__":
    sst = datetime.now()
    main()
    # check_answer_differences()
    # complete_answer_based_on_differences()
