...
def load_data_with_doc(filename, num_docs):
    ...
    with io.open(filename,'r', encoding='utf-8') as f:
        # for line in tqdm(f, total=87291):
        for line in f:
...
def read_data(filename, keys):
    ...
    for line in open(filename):
        ...
        if ('squad' in filename in filename):
            answers = data['answer_tok'] # [tokenize_text(a).words() for a in data['answer']]
        else:
            if ('CuratedTrec' in filename):
                answers = data['answer']
            else:
                answers = data['answers'] # [tokenize_text(a).words() for a in data['answers']]
                answers_tok = data['answers_tok']
        # question = " ".join(tokenize_text(data['question']).words())
        question = data['question']
        question_tok = data["question_tok"]
        res.append({"answers":answers, "question":question, 'answers_tok': answers_tok, 'question_tok': question_tok, "answers_orig": data['answers']})
        step+=1
    return res


def load_list_with_doc(data_lst, num_docs):
    """Load examples from preprocessed file.
    One example per line, JSON encoded.
    """
    # Load JSON lines
...
def read_list(data_lst, args):
    ...
    for data in data_lst:
        if ('squad' in args.dataset in args.dataset):
            answers = data['answer'] # [tokenize_text(a).words() for a in data['answer']]
        else:
            if ('CuratedTrec' in args.dataset):
                ...
                answers_tok = data['answers_tok']
        # question = " ".join(tokenize_text(data['question']).words())
        question = data['question'] # " ".join(data['question_tok'])
        question_tok = data['question_tok']
        res.append({"answers":answers, "question":question, "answers_tok": answers_tok, "question_tok": question_tok})
        step+=1
    return res
